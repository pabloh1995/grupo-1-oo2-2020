...
public class LocalConverter {
	...
	public LocalModel entityToModel(Local local) {
		return new LocalModel(local.getIdLocal(), local.getTelefono(), local.getDireccion(), local.getLatitud(),
				local.getLongitud());
	}

	public Local modelToEntity(LocalModel localModel) {

		return new Local(localModel.getIdLocal(), localModel.getTelefono(), localModel.getDireccion(),
				localModel.getLatitud(), localModel.getLongitud());
	}
}