...
public class VentaConverter {
	...
	public VentaModel entityToModel(Venta venta) {
		return new VentaModel(venta.getIdVenta(), venta.getFechaVenta(),
				empleadoConverter.entityToModel(venta.getEmpleado()),
				clienteConverter.entityToModel(venta.getCliente()), localConverter.entityToModel(venta.getLocal()),
				venta.getTotal());
	}

	public Venta modelToEntity(VentaModel ventaModel) {
		return new Venta(ventaModel.getIdVenta(), ventaModel.getFechaVenta(),
				empleadoConverter.modelToEntity(ventaModel.getEmpleado()),
				clienteConverter.modelToEntity(ventaModel.getCliente()),
				localConverter.modelToEntity(ventaModel.getLocal()), ventaModel.getTotal());
	}
