...
public class SolicitudStockService implements ISolicitudStockService{
	...
	public void setAttributes(SolicitudStockModel solicitudStockModel) {
		...
		solicitudStockModel.setLocal(localService.findByIdLocal(empleadoService.findByIdPersona(pedidoService.getEmpleadoSS().getIdPersona()).getLocal().getIdLocal()));
		solicitudStockModel.setProducto(productoService.findByIdProducto(pedidoService.getProductoSS().getIdProducto()));
	...
	public SolicitudStockModel update(SolicitudStockModel solicitudStockModel) {
		...
		solicitudStockModel.setEmpleado(empleadoService.findByIdPersona(pedidoService.findByIdPedido(solicitudStockModel.getPedido().getIdPedido()).getEmpleado().getIdPersona()));
		solicitudStockModel.setLocal(localService.findByIdLocal(empleadoService.findByIdPersona(pedidoService.findByIdPedido(solicitudStockModel.getPedido().getIdPedido()).getEmpleado().getIdPersona()).getLocal().getIdLocal()));
		solicitudStockModel.setLocal2(localService.findByIdLocal(solicitudStockModel.getLocal2().getIdLocal()));
