...
public class EmpleadoService implements IEmpleadoService{
	...
	public List<SueldosModel> SueldoBruto() {
		List<SueldosModel> sm = new ArrayList<SueldosModel>();
		List<EmpleadoModel> e = new ArrayList<EmpleadoModel>();
		SueldosModel smAux = new SueldosModel();
		e = this.getAlls();
		int i=0;
		for(i=0; i< e.size(); i++) {
			smAux.setEmpleado(e.get(i));
			smAux.setSueldoBruto(smAux.getEmpleado().getSueldoBasico()+smAux.getEmpleado().getComision());
			sm.add(smAux);
		}
		return sm;
	}
