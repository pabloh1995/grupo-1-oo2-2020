...
public class LoteConverter {
	...
	public LoteModel entityToModel(Lote lote) {
		return new LoteModel(lote.getIdLote(), lote.getFechaIngreso(), lote.getCantidadIngreso(),
				lote.getCantidadActual(), lote.isActivo(), productoConverter.entityToModel(lote.getProducto()),
				localConverter.entityToModel(lote.getLocal()));
	}

	public Lote modelToEntity(LoteModel loteModel) {
		return new Lote(loteModel.getIdLote(), loteModel.getFechaIngreso(), loteModel.getCantidadIngreso(),
				loteModel.getCantidadActual(), loteModel.isActivo(),
				productoConverter.modelToEntity(loteModel.getProducto()),
				localConverter.modelToEntity(loteModel.getLocal()));
	}

	/*
	 * public List<LoteModel> loteToLoteModel (List<Lote> lote) { List<LoteModel>
	 * nuevoLotes = new ArrayList<LoteModel>(); for(int i = 0 ; i < lote.size(); i
	 ...
	 * loteEntidad.setProducto(productoConverter.entityToModel(lote.get(i).
	 * getProducto())); loteEntidad.setStock(lote.get(i).getStock());
