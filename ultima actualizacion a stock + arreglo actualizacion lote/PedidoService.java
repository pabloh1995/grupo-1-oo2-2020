...
public class PedidoService implements IPedidoService {
	...
	public List<LocalModel> LocalesConStock() {
		...
		while(i<pedidos.size()) {
			...
			if(pedidos.get(i).isResuelto()==false ){
				if((pedidos.get(i).getIdPedido()==solicitudes.get(k).getPedido().getIdPedido() && solicitudes.get(k).isActivo()==false && solicitudes.get(k).isAceptado()==true)||
						pedidos.get(i).getIdPedido()==solicitudes.get(k).getPedido().getIdPedido() && solicitudes.get(k).isActivo()==true) {
		...
		while(i<localesConStock.size()) {
			...
			va1 = Math.pow(sindLat, 2)	+ Math.pow(sindLng, 2) * Math.cos(Math.toRadians(pedidoAux.getLocal().getLatitud())) * Math.cos(Math.toRadians(localesConStock.get(i).getLatitud()));
	...
	public void setDetalleVenta(VentaModel ventaModel) {
		...
		float total =0;
		while (i<pedidos.size()) {
			if(pedidos.get(i).getCliente().getIdPersona()==ventaModel.getCliente().getIdPersona() && pedidos.get(i).getEmpleado().getIdPersona()==ventaModel.getEmpleado().getIdPersona() && pedidos.get(i).isResuelto()==true) {
			detalleVenta.setProducto(pedidos.get(i).getProducto());
			detalleVenta.setCantidad(pedidos.get(i).getCantidad());
			detalleVenta.setPrecioSubTotal(pedidos.get(i).getCantidad()*pedidos.get(i).getProducto().getPrecioUnitario());
		...
		else if (pedidos.get(i).getCliente().getIdPersona()==ventaModel.getCliente().getIdPersona() && pedidos.get(i).getEmpleado().getIdPersona()==ventaModel.getEmpleado().getIdPersona() && pedidos.get(i).isResuelto()==false){
			solicitudes = solicitudStockService.getAlls();
			while(j<solicitudes.size()) {
				if(solicitudes.get(j).getEmpleado().getIdPersona()==pedidos.get(i).getEmpleado().getIdPersona()&& solicitudes.get(j).getCantidad()==pedidos.get(i).getCantidad() && solicitudes.get(j).getProducto().getIdProducto()== pedidos.get(i).getProducto().getIdProducto() && solicitudes.get(j).isAceptado()==true) {
					detalleVenta.setProducto(pedidos.get(i).getProducto());
					detalleVenta.setCantidad(pedidos.get(i).getCantidad());
					detalleVenta.setPrecioSubTotal(pedidos.get(i).getCantidad()*pedidos.get(i).getProducto().getPrecioUnitario());
					detalleVenta.setVenta(ventaService.findByIdVenta(ventaModel.getIdVenta()));
					...
					e.setComision(e.getComision()+solicitudes.get(j).getCantidad()*solicitudes.get(j).getProducto().getPrecioUnitario()*0.03f);
